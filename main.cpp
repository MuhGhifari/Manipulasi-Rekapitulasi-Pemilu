...
void tambahSuara(int& suaraKandidat, int& suaraTotal, int min, int max);
  ...
  while (getline(fileInput, str)) {
    ...
    for (size_t i = 0; i < kandidat.size(); i++) {
      ...
      if (kandidat[i] == "Paslon 02" && totalSuaraTps < batasMaksimalSuaraTps) {
        ...
        // memanggil fungsi tambahSuara
        tambahSuara(suaraKandidat[kandidat[i]], totalSuaraTps, minimal, maksimal);
      }
    }
...
/*
Function untuk menambah suarajumlah suara
*/
void tambahSuara(int& suaraKandidat, int& suaraTotal, int min, int max) {

  // variabel untuk inisialisasi seed ke random generator
  random_device acak;

  // mendefinisikan generator nomor acak dengan metode "Mersenne Twister"
  mt19937 gen(acak());

  // menyatakan distribusi yg diinginkan adalah dari angka min sampai max
  uniform_int_distribution<int> distribusi(min, max);

  // menghasilkan nomor acak antara min dan max
  int suaraTambahan = distribusi(gen);

  // menambahkan suara tambahan pada variabel pointer
  suaraKandidat += suaraTambahan;
  suaraTotal += suaraTambahan;
}
